...
RaidNexus.SimCExport = RaidNexus.SimCExport or {}

local Simulationcraft = RaidNexus.SimCExport
local realmInfoLib = nil
if LibStub then
  realmInfoLib = LibStub("LibRealmInfo", true)
end
...
local COMPAT_EXPORT_VERSION = "0.2.31"
...
local function EnsureOptionsDB()
  RaidNexusDB = RaidNexusDB or {}
  RaidNexusDB.simc = RaidNexusDB.simc or {}
  copyDefaults(RaidNexusDB.simc, defaultOptions)
  return RaidNexusDB.simc
end

local function getLinks(input)
  local separatedLinks = {}
  for link in (input or ""):gmatch("|c.-|h|r") do
     separatedLinks[#separatedLinks + 1] = link
  end
  return separatedLinks
end

local function GetExportVersion()
  if RaidNexus and RaidNexus.version and RaidNexus.version ~= "" then
    return RaidNexus.version
  end

  return COMPAT_EXPORT_VERSION
end

function Simulationcraft:HandleChatCommand(input)
  EnsureOptionsDB()
  local args = {strsplit(' ', input or "")}
  ...
  for _, arg in ipairs(args) do
    ...
    elseif arg == 'minimap' then
      RaidNexusDB = RaidNexusDB or {}
      RaidNexusDB.minimap = RaidNexusDB.minimap or {}
      RaidNexusDB.minimap.hide = not RaidNexusDB.minimap.hide
      if RaidNexus.UpdateMinimapButtonVisibility then
        RaidNexus:UpdateMinimapButtonVisibility()
      end
      RaidNexus:Print(
        "SimC export minimap button is now " .. (RaidNexusDB.minimap.hide and "hidden" or "shown")
      )
      return
    end
  end
...
function Simulationcraft:GetMainFrame(text)
  ...
  if not SimcFrame then
    -- Main Frame
    local frameConfig = EnsureOptionsDB().profile.frame
    local f = CreateFrame("Frame", "RaidNexusSimcFrame", UIParent, "DialogBoxFrame")
    f:ClearAllPoints()
    ...
    -- scroll frame
    local sf = CreateFrame("ScrollFrame", "RaidNexusSimcScrollFrame", f, "UIPanelScrollFrameTemplate")
    sf:SetPoint("LEFT", 16, 0)
    sf:SetPoint("RIGHT", -32, 0)
    sf:SetPoint("TOP", 0, -32)
    sf:SetPoint("BOTTOM", _G[f:GetName() .. "Button"], "TOP", 0, 0)

    -- edit box
    local ctrlDown = false
    local eb = CreateFrame("EditBox", "RaidNexusSimcEditBox", RaidNexusSimcScrollFrame)
    ...
    eb:SetScript("OnKeyUp", function(self, key)
      ...
      if ctrlDown then
        ...
        if key == "C" or key == "X" then
          if EnsureOptionsDB().profile.closeOnCopy then
    ...
    local rb = CreateFrame("Button", "RaidNexusSimcResizeButton", f)
    ...
    local checkbox = CreateFrame("CheckButton", "RaidNexusSimcAutomaticClose", f, "ChatConfigCheckButtonTemplate")
    checkbox:SetPoint("BOTTOMLEFT", 12, 18)
    checkbox.Text:SetText("Close after copy")
    checkbox:SetChecked(EnsureOptionsDB().profile.closeOnCopy)
    checkbox:HookScript("OnClick", function(self)
      EnsureOptionsDB().profile.closeOnCopy = self:GetChecked()
    end)

    SimcFrame = f
  end
  RaidNexusSimcEditBox:SetText(text)
  RaidNexusSimcEditBox:HighlightText()
  return SimcFrame
end
...
function Simulationcraft:GetSimcProfile(debugOutput, noBags, showMerchant, links)
  -- addon metadata
  local versionComment = '# RaidNexus SimC Export ' .. GetExportVersion()
  local wowVersion, wowBuild, _, wowToc = GetBuildInfo()
  local wowVersionComment = '# WoW ' .. wowVersion .. '.' .. wowBuild .. ', TOC ' .. wowToc
  local simcVersionWarning = '# SimulationCraft-compatible export generated by RaidNexus'

  -- Basic player info
  local realmName, region, realmLatinName
  if realmInfoLib and realmInfoLib.GetRealmInfoByUnit then
    local _, libRealmName, _, _, _, _, libRegion, _, _, libRealmLatinName = realmInfoLib:GetRealmInfoByUnit('player')
    realmName = libRealmName
    region = libRegion
    realmLatinName = libRealmLatinName
  end
  ...
  elseif ClassTalents then
    ...
    if Traits.GetLoadoutSerializationVersion() ~= SUPPORTED_LOADOUT_SERIALIZATION_VERSION then
      simcPrintError = 'This version of the RaidNexus SimC export does not work with this version of WoW.\n'
